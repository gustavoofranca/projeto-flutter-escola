...
// Flag temporária para sempre exibir o botão de criar avisos
const bool kForceAnnouncementCreateVisible = true;
...
class _AnnouncementsScreenState extends State<AnnouncementsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (kForceAnnouncementCreateVisible || _canCreateOrEdit(user))
            IconButton(
      ...
      floatingActionButton: (kForceAnnouncementCreateVisible || _canCreateOrEdit(user))
    ...
    return Padding(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          border: Border(
            left: BorderSide(width: 4, color: announcement.priorityColor),
          ),
          boxShadow: [
            BoxShadow(
  ...
  bool _canCreateOrEdit(user, [AnnouncementModel? a]) {
    if (user == null) return false;
    // Admin pode tudo
    if (user.userType.name == 'admin') return true;
    // Professor pode criar e editar apenas seus próprios avisos
    if (user.userType.name == 'teacher') {
      if (a == null) return true; // criar
      return a.teacherId == user.id; // editar
    }
    // Aluno não pode
    return false;
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppDimensions.lg),
          CustomTypography.h6(text: message, color: AppColors.textPrimary),
          const SizedBox(height: AppDimensions.sm),
