...
class AnnouncementProvider extends ChangeNotifier {
  ...
  List<AnnouncementModel> getAnnouncementsForUser(UserModel user) {
    if (user.userType == UserType.teacher) {
      ...
      return _announcements.where((a) => a.classId == user.classId || a.classId == null).toList();
    } else {
  ...
  List<AnnouncementModel> getUrgentAnnouncements(UserModel user) {
    return getAnnouncementsForUser(user)
        .where((a) => a.priority == AnnouncementPriority.urgent || a.priority == AnnouncementPriority.high)
        .toList();
  }

  /// Load announcements (simulate API call)
    ...
    try {
      ...
      _announcements.addAll([
        AnnouncementModel(
          ...
          content: 'A prova será realizada na próxima segunda-feira, dia 28/08, às 14h. Conteúdo: Equações do 2º grau, função quadrática e sistemas lineares. Não esqueçam de trazer calculadora científica.',
        ...
        AnnouncementModel(
          ...
          content: 'Reunião marcada para sexta-feira, 30/08, às 19h no auditório principal. Serão discutidos o desempenho da turma, projetos do próximo bimestre e esclarecimentos sobre o sistema de avaliação.',
        ...
        AnnouncementModel(
          ...
          content: 'Entrega do trabalho sobre a República Velha no Brasil até quinta-feira, 01/09, às 23h59 via plataforma online. O trabalho deve ter no mínimo 8 páginas, incluir bibliografia e seguir as normas ABNT.',
          teacherId: 'teacher_002',
        ...
        AnnouncementModel(
          ...
          content: 'A aula de Educação Física de hoje (25/08) foi cancelada devido às condições climáticas adversas. A reposição será agendada para a próxima terça-feira no mesmo horário.',
        ...
        AnnouncementModel(
          ...
          content: 'A Feira de Ciências será realizada no dia 15 de dezembro no ginásio da escola. Inscrições de projetos até 10/12. Categorias: Ciências da Natureza, Matemática, Tecnologia e Sustentabilidade.',
          teacherId: 'teacher_001',
        ...
        AnnouncementModel(
          ...
          content: 'Lembrete para todos os alunos: a entrega dos materiais de laboratório de Química deve ser feita até sexta-feira na sala dos professores. Verificar se todos os itens estão limpos e organizados.',
  ...
  int getUnreadCount(UserModel user) {
    ...
    return getAnnouncementsForUser(user)
        .where((a) => a.createdAt.isAfter(DateTime.now().subtract(const Duration(days: 7))))
        .length;
  }

  /// Filter announcements by type
  List<AnnouncementModel> filterByType(List<AnnouncementModel> announcements, AnnouncementType type) {
    return announcements.where((a) => a.type == type).toList();
  }

  /// Filter announcements by priority
  List<AnnouncementModel> filterByPriority(List<AnnouncementModel> announcements, AnnouncementPriority priority) {
    return announcements.where((a) => a.priority == priority).toList();
  }

  /// Search announcements
  List<AnnouncementModel> searchAnnouncements(List<AnnouncementModel> announcements, String query) {
